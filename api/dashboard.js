export default async function handler(req, res) {
  ...
  const weightData = [{date:'Jan 1',w:120.2},{date:'Jan 8',w:118.2},{date:'Jan 15',w:118.8},{date:'Jan 22',w:117.0},{date:'Jan 29',w:116.8},{date:'Feb 5',w:117.6},{date:'Feb 12',w:118.8},{date:'Feb 19',w:117.0},{date:'Feb 26',w:119.4},{date:'Mar 5',w:120.6},{date:'Mar 12',w:119.6},{date:'Mar 19',w:119.2},{date:'Mar 26',w:121.6},{date:'Apr 2',w:121.2}];
  ...
  try {
    ...
    res.send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Claire Health Dashboard</title><script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"><\/script><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f7f7f5;padding:2rem;color:#111;}h1{font-size:22px;font-weight:500;margin-bottom:4px;}.sub{font-size:13px;color:#888;margin-bottom:24px;}.section{font-size:11px;font-weight:500;color:#999;text-transform:uppercase;letter-spacing:0.06em;margin:24px 0 10px;}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:8px;}.card{background:#fff;border-radius:12px;padding:1rem 1.25rem;border:0.5px solid #e8e8e8;}.label{font-size:12px;color:#888;margin-bottom:4px;}.value{font-size:22px;font-weight:500;}.unit{font-size:13px;color:#aaa;margin-left:2px;}.chart-wrap{background:#fff;border-radius:12px;border:0.5px solid #e8e8e8;padding:1.25rem;margin-bottom:8px;}.row{display:flex;justify-content:space-between;padding:7px 0;border-bottom:0.5px solid #f0f0f0;font-size:13px;}.row:last-child{border-bottom:none;}.good{color:#1D9E75;}.footer{font-size:12px;color:#aaa;margin-top:24px;text-align:center;}</style></head><body>'
    ...
    + 'new Chart(document.getElementById("weightChart"),{type:"line",data:{labels:wL,datasets:[{label:"Weight",data:wV,borderColor:"#378ADD",backgroundColor:"rgba(55,138,221,0.07)",tension:0.3,pointRadius:4,pointBackgroundColor:ptC,fill:true}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{min:114,max:125,ticks:{callback:v=>v+" lb"}},x:{ticks:{maxRotation:45,autoSkip:false},grid:{display:false}}}}});'
    + '<\/script></body></html>');
  } catch(err) {
